...
path = 'GUIXML-Output-Data'
try:
    os.mkdir(path)
except FileExistsError:
    print('please delete or rename old folder and try again')
...
for i in range(0, len(filenames)):
    ...
    with open(xml) as fxml:
        ...
        highlight = highlight.save(path + img.replace('.png', '-annotated.png').replace(sys.argv[1], ''))
